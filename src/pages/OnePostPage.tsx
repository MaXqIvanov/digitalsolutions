...
export const OnePostPage = () => {
  ...
  const urlElelement = urlElements[2];
  useEffect(() => {
    ...
    if (onePost?.title == undefined) {
      axios
        .get(`https://jsonplaceholder.typicode.com/posts/${urlElelement}`)
        .then((elem: any) => dispatch(getOnePost(elem.data)));
    }
  }, [loadData]);
  useEffect(() => {
    axios
      .get(`https://jsonplaceholder.typicode.com/comments?postId=${urlElelement}`)
      .then((elem: any) => dispatch(getComments(elem.data)));
  }, []);
  ...
  return (
    <div className={s.main}>
      <div className={s.main_wrap}>
        <div className={s.block_info_user}>{user?.name}</div>
        <div className={s.block_posts}>
          <div className={s.block_posts_wrap}>
            <div className={s.block_post_title}>
              <span>{onePost?.title}</span>
            </div>
            <div className={s.block_post_body}>
              <span>{onePost?.body}</span>
            </div>
          </div>
        </div>
        <div className={s.block_comments}>
          ...
          <div>
            {comments ? (
              comments.map((elem: any) => <OneComment comment={elem} />)
            ) : (
        ...
        <div className={s.comment_block_btn}>
          <div onClick={() => setIsForm(!isForm)} className={s.comment_button}>
            <span>Добавить комментарий</span>
          </div>
        </div>

        {isForm ? (
          <FormSend urlElelement={urlElelement} setIsForm={setIsForm} isForm={isForm} />
        ) : (
          <></>
        )}
      </div>
