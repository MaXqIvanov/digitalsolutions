...
export const UserInfo = () => {
  ...
  useEffect(() => {
    if (user?.name == undefined) {
      ...
      const urlElelement = urlElements[1];
    ...
    <div>
      ...
      <div className={s.block_user_bottomLine}>
        <div style={{ overflow: 'hidden' }} className={s.block_user_bottomLine_main}>
          <div className={s.block_user_city}>
            <span>{user?.address?.city}</span>
          </div>
          <div className={s.block_user_mail}>
            <span>{user?.email}</span>
          </div>
          <div className={s.block_user_phone}>
            <span>{user?.phone}</span>
          </div>
          <div className={s.block_user_site}>
            <span>web-site:{user?.website}</span>
          </div>
          <div className={s.block_user_company}>
